    ...
    return {
        ...
        generateBundle(_, bundle) {
            ...
            const grantFunctionNameList = Object.values(UserScript)
                ...
                .map(([_, value]) => value);
            const grantFunctionNameSet = new Set(grantFunctionNameList);
            for (const fileName in bundle) {
                const chunk = bundle[fileName];
                if (chunk.type !== 'chunk')
                    continue;
                // 使用正则匹配所有可能的 GM_* / CAT_* 调用（简单高效）
                const grantMatches = new Set();
...
export const appendPromisePlugin = (isEnabled) => {
    if (!isEnabled)
        return { name: 'noop' };
    return {
 ...
 * 插件 3: 输出 banner
 */
export const bannerPlugin = (content) => {
    return {
 ...
 * 插件4: 备份生产版本文件
 */
export const backupPlugin = (version) => {
    return {
