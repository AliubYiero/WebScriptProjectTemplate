 ...
 * 顶部信息字符串化
 */
const stringifyBannerContent = (isProduction, outputFileName) => {
    ...
    * */
    ...
    // 获取所有 key 中的最大长度
    const UserScriptKeyMaxLength = userScript.reduce((maxLength, item) => Math.max(item[0].length, maxLength), 0);
    // 如果是 开发环境, 在 UserScript 中加入 require 自我引用
    const selfRequire = ['require', pathToFileURL(resolve(__dirname, 'dist', outputFileName)).href];
    ...
    bannerContentList.push(...[
        ...
        userScript.map(([key, value]) => {
            ...
            const formatValue = ['version', 'name', 'name:en'].includes(key) && !isProduction
                ? `${value}-beta`
                : value;
...
const AppendPromiseBodyPlugin = (options = {}) => {
    const { padStartContent = '', padEndContent = '', backgroundScript = false, } = options;
    let viteConfig;
    // Handle files
    return {
...
export default defineConfig(({ mode }) => {
    ...
    * */
     ...
     * 将 UserScript 转换成对象
     */
    const userScript = UserScript.reduce((userScript, [key, value]) => {
    ...
    * */
     ...
     * 判断是否存在必要信息
     */
    ['name', 'version', 'description', 'author'].forEach(key => {
        if (!userScript[key]) {
