 ...
 */
export const parseUserScript = (userScript, isDevelopment = false) => {
    // 获取所有 key 中的最大长度
    const userScriptKeyMaxLength = userScript.reduce((maxLength, item) => Math.max(item[0].length, maxLength), 0);
    // 记录只需要 key 值的属性
    const booleanKeySet = new Set(['background', 'early-start', 'noframes']);
    // 过滤掉空属性
    const filterUserScript = userScript.filter(([key, value]) => booleanKeySet.has(key) || value);
    // 将 Entry 数组形式的 UserScript 转换成对象
    const userScriptObject = Object.fromEntries(filterUserScript);
    // 如果是 开发环境, 在 UserScript 中加入 require 自我引用
    const outputFileName = userScriptObject.name + '.dev.js';
    isDevelopment && filterUserScript.push(['require', pathToFileURL(resolve(__dirname, 'dist', outputFileName)).href]);
    return {
 ...
 * 字符串化 UserScript
 */
export const stringifyUserScript = (userScript) => {
    return [
        '// ==UserScript==',
 ...
 * 字符串化 UserConfig
 */
export const stringifyUserConfig = (userConfig) => {
 ...
 * 输入校验
 */
export const validateUserScript = (userScript, isProduction) => {
